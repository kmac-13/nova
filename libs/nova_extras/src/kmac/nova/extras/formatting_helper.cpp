...
/**
 * @brief Look-up table for 2-digit strings: "00" to "99".
 */
...
/**
 * @brief Look-up table for 3-digit strings: "000" to "999".
 */
// NOLINT NOTE: 2D string literal lookup table; using std::array<std::array> would be cumbersome
...
char* FormattingHelper::formatTimestamp( char* out, std::uint64_t timestamp ) noexcept
{
	...
	out += 4;
	*out++ = '-';
	std::memcpy( out, detail::DIGITS_2[ time.tm_mon + 1 ], 2 );
	out += 2;
	*out++ = '-';
	std::memcpy( out, detail::DIGITS_2[ time.tm_mday ], 2 );
	out += 2;
	...
	*out++ = 'T';
	...
	// HH:MM:SS.mmm
	std::memcpy( out, detail::DIGITS_2[ time.tm_hour ], 2 );
	out += 2;
	*out++ = ':';
	std::memcpy( out, detail::DIGITS_2[ time.tm_min ], 2 );
	out += 2;
	*out++ = ':';
	std::memcpy( out, detail::DIGITS_2[ time.tm_sec ], 2 );
	out += 2;
	*out++ = '.';
...
char* FormattingHelper::formatTagId( char* out, std::uint64_t tagId ) noexcept
{
