...
static constexpr const char JSON_DIGITS_2[ 100 ][ 3 ] = {
...
// NOLINTNEXTLINE(cppcoreguidelines-avoid-c-arrays)
static constexpr const char JSON_DIGITS_3[ 1000 ][ 4 ] = {
...
// NOLINTNEXTLINE(cppcoreguidelines-avoid-c-arrays)
static constexpr const char HEX_CHARS[ 16 ] = {
...
void JsonFormatter::buildTs( std::uint64_t timestamp ) noexcept
{
	...
	out += 4;
	*out++ = '-';
	std::memcpy( out, detail::JSON_DIGITS_2[ time.tm_mon + 1 ], 2 );
	out += 2;
	*out++ = '-';
	std::memcpy( out, detail::JSON_DIGITS_2[ time.tm_mday ], 2 );
	out += 2;
	*out++ = 'T';
	std::memcpy( out, detail::JSON_DIGITS_2[ time.tm_hour ], 2 );
	out += 2;
	*out++ = ':';
	std::memcpy( out, detail::JSON_DIGITS_2[ time.tm_min ], 2 );
	out += 2;
	*out++ = ':';
	std::memcpy( out, detail::JSON_DIGITS_2[ time.tm_sec ], 2 );
	out += 2;
	*out++ = '.';
	std::memcpy( out, detail::JSON_DIGITS_3[ millis ], 3 );
	out += 3;
	*out++ = 'Z';
	*out++ = '"';
