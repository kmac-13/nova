...
// lookup tables kept self-contained per formatter header to avoid
// cross-header dependencies

// NOLINTNEXTLINE(cppcoreguidelines-avoid-c-arrays)
static constexpr const char XML_DIGITS_2[ 100 ][ 3 ] = {
...
// NOLINTNEXTLINE(cppcoreguidelines-avoid-c-arrays)
static constexpr const char XML_DIGITS_3[ 1000 ][ 4 ] = {
...
{
	const std::uint64_t seconds = timestamp / std::uint64_t( 1000000000 );
	const std::uint64_t millis = ( timestamp / std::uint64_t( 1000000 ) ) % 1000;

	const auto timeVal = static_cast< std::time_t >( seconds );
	std::tm time {};
	...
	out += 4;
	*out++ = '-';
	std::memcpy( out, detail::XML_DIGITS_2[ time.tm_mon + 1 ], 2 );
	out += 2;
	*out++ = '-';
	std::memcpy( out, detail::XML_DIGITS_2[ time.tm_mday ], 2 );
	out += 2;
	*out++ = 'T';
	std::memcpy( out, detail::XML_DIGITS_2[ time.tm_hour ], 2 );
	out += 2;
	*out++ = ':';
	std::memcpy( out, detail::XML_DIGITS_2[ time.tm_min ], 2 );
	out += 2;
	*out++ = ':';
	std::memcpy( out, detail::XML_DIGITS_2[ time.tm_sec ], 2 );
	out += 2;
	*out++ = '.';
	std::memcpy( out, detail::XML_DIGITS_3[ millis ], 3 );
	out += 3;
	*out++ = 'Z';
...
{
	static constexpr const char HEX[ 16 ] = {  // NOLINT(cppcoreguidelines-avoid-c-arrays)
	...
	{
		*out++ = HEX[ ( tagId >> static_cast< unsigned >( i ) ) & 0xFu ];
	}
