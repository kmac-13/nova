...
void semihostingExit( int exitCode )
{
	...
	// QEMU maps ADP_Stopped_ApplicationExit to host exit(0); any other reason
	// code maps to host exit(1)
	// SYS_EXIT_EXTENDED (0x20) supports arbitrary exit codes via a block,
	// but requires probing for the extension first
	// since CI only needs pass/fail, using the two standard reason codes 
	// is sufficient
