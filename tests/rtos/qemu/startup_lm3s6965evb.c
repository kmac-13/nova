/*
 * startup_lm3s6965evb.c - minimal Cortex-M3 startup for lm3s6965evb under QEMU
 *
 * Provides:
 ...
 *
 * FreeRTOS overrides SVC_Handler, PendSV_Handler, and SysTick_Handler via
 * the macros in FreeRTOSConfig.h (vPortSVCHandler, xPortPendSVHandler,
 * xPortSysTickHandler).  All other slots fall through to Default_Handler.
 *
 * This file is compiled only for the ARM cross-compile target
 * (NOVA_RTOS_QEMU=ON in tests/rtos/CMakeLists.txt).
 */
